...
bankmodel = function (input) {
  ...
  for(i in 1:nrow(input)){
    ...
    #--------------------------#
      ...
      #-----------------#
      loss.stats = losses(npmt, input$chance.full.loss[i],input$recovery[i],input$LSR[i])
            

      loan.payment = pmt.from.risk()
...
#--------------------------------#
  ...
  #--------------------------------#
  # calculate loan payment given
  # risk information and loan amount
  #--------------------------------#
  pmt.from.risk = function(input, i, no.default.chance, loan.amt,
                          expected.loss.noLLR= NULL){
    ...
    ev.pmt = no.default.chance + ((1-no.default.chance) * 
                                  (1-(1-input$recovery[i]) *(1-(input$LSR[i]* small.LLR))))
  ...
  #--------------------------------#
  # calculate of no default over tenor
  # and value of expected loss
  # return as a paired list
  #--------------------------------#
  losses = function (npmt, chance.full.loss, recovery, LSR){
    output = list()
    
    k = seq(1,npmt,by=1) #list of numbers from 1 to npmt
    
    # chance of default in a given month
    # assume a uniform chance of default over the whole loan tenor
    output$default.chance.mo = sub.prob(chance.full.loss,npmt) #ie chance of default in a given payment cycle (month)
    # cumulative chance of no default by a given month
    output$no.default.chance = (1-default.chance.mo) ^ k #this makes a list with length 12*tenor=npmt
    
    # for bookkeeping, what % of total value is expected to be lost?    
    output$expected.loss = sum((1-no.default.chance)*((1-recovery) *(1-LSR)))/(npmt)
    output$expected.loss.noLLR = sum((1-no.default.chance)*(1-recovery))/(npmt)
