...
scenario = 'highRisk.newPremia' #what set of params do you want?
...
#     inlist$gvt.discount = .04#seq(.01,.05,by=.01) 
  ...
  # risky info #
  #------------#
      inlist$risk.adjust =c(seq(.5,2,by=.5),1.4) # as in (Amato, J., 2005) this number is used as follows
...
#       inlist$LPCR = seq(0,.4,by=.01)#c(.02,.03,.04,.05,.06,.07) # loan pool coverage ratio. usually around 5-10%
